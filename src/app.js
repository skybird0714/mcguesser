...
function getApiKey() {
    try {
        ...
        // 方式4: 通过process.env对象 (如果可用)
        if (typeof process !== 'undefined' && process.env && process.env.OPENROUTER_API_KEY) {
            console.log("通过process.env获取到API密钥");
            return process.env.OPENROUTER_API_KEY;
        }
        
        // 如果无法获取API密钥，显示错误信息
        console.error("无法获取API密钥，请确保在Cloudflare Pages的'变量与机密'中设置了OPENROUTER_API_KEY环境变量");
        displayErrorMessage("无法获取API密钥，请联系管理员");
        return '';
    } catch (error) {
        console.error("获取API密钥时出错:", error);
        displayErrorMessage("获取API密钥时出错，请联系管理员");
        return '';
    }
}
