...
async function sendInitialPrompt() {
    ...
    const systemPrompt = `你是一个Minecraft物品猜谜游戏的AI。游戏规则：你想一个截止到你数据库结束的Minecraft物品（必须是具体的，例如钻石头盔，不能头盔），用户会进行多轮对话询问你的特性,，你只能回答"是"、"不是"或"不确定"。如果用户直接猜出物品名称，且猜对了，你应该回答"猜测正确！"，否则你应该回答"不是，请继续猜测",保持回答简短，不要提供额外信息， 如果用户放弃，请直接告诉他们正确答案。开始时，请说"我想好了一个Minecraft物品，请开始猜测！"`;
...
async function sendMessage() {
    ...
    try {
        // 如果已达到最大回合数，直接让AI给出答案
        if (currentRound >= 20) {
            // 添加请求给出答案的消息
            conversationHistory.push({
                role: "user",
                content: "已经猜测了20次，请直接告诉我正确答案是什么"
            });
        }
        ...
        if (data && data.choices && data.choices.length > 0 && data.choices[0].message) {
            ...
            // 检查游戏是否结束
            if (finalMessage.includes("猜测正确") || currentRound >= 20) {
                endGame();
            } else {
                // 启用输入
                enableUserInput();
...
function updateRoundCount() {
    ...
    if (currentRound >= 17) {
        // 红色警告
        roundCount.style.color = '#ff4d4d';
        roundCount.style.fontWeight = 'bold';
    } else if (currentRound >= 12) {
        // 黄色警告
        roundCount.style.color = '#ffcc00';
        roundCount.style.fontWeight = 'bold';
    } else {
        // 恢复默认颜色
        roundCount.style.color = '';
        roundCount.style.fontWeight = '';
    }
}
