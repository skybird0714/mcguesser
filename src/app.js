...
async function sendInitialPrompt() {
    ...
    try {
        ...
        displayErrorMessage("游戏启动失败，但你仍然可以继续玩。");
...
async function sendMessage() {
    ...
    try {
        ...
        displayErrorMessage("发送消息失败，但游戏可以继续。");
...
async function giveUp() {
    ...
    try {
        ...
        displayErrorMessage("放弃游戏失败，但游戏已结束。");
    }
...
// 从环境变量获取API密钥
function getApiKey() {
    // 在Cloudflare Pages中，环境变量通过window对象访问
    try {
        ...
        // 尝试直接访问（某些Cloudflare Pages配置）
        if (typeof OPENROUTER_API_KEY !== 'undefined') {
            return OPENROUTER_API_KEY;
        }
        
        console.error("无法获取API密钥，请确保在Cloudflare Pages中设置了OPENROUTER_API_KEY环境变量");
        return '';
    } catch (error) {
